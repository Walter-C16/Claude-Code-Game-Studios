...
##   AC2 — get_profile("artemisa") returns dict with all 10 required keys
...
const EXPECTED_IDS: Array[String] = ["artemisa", "hipolita", "atenea", "nyx", "priestess"]
const COMPANION_IDS: Array[String] = ["artemisa", "hipolita", "atenea", "nyx"]
...
# ── AC2 — get_profile("artemisa") has all 10 required keys ───────────────────

func test_companion_registry_get_profile_artemisa_has_id_key() -> void:
	# Arrange
	var registry = _make_registry()

	# Act
	var profile: Dictionary = registry.get_profile("artemisa")

	# Assert
	assert_bool(profile.has("id")).is_true()

func test_companion_registry_get_profile_artemisa_has_display_name_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("display_name")).is_true()

func test_companion_registry_get_profile_artemisa_has_role_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("role")).is_true()

func test_companion_registry_get_profile_artemisa_has_element_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("element")).is_true()

func test_companion_registry_get_profile_artemisa_has_str_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("STR")).is_true()

func test_companion_registry_get_profile_artemisa_has_int_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("INT")).is_true()

func test_companion_registry_get_profile_artemisa_has_agi_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("AGI")).is_true()

func test_companion_registry_get_profile_artemisa_has_card_value_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("card_value")).is_true()

func test_companion_registry_get_profile_artemisa_has_starting_location_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("starting_location")).is_true()

func test_companion_registry_get_profile_artemisa_has_portrait_path_base_key() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
	assert_bool(profile.has("portrait_path_base")).is_true()

func test_companion_registry_get_profile_artemisa_stat_str_is_correct() -> void:
	# Arrange
	var registry = _make_registry()
...
func test_companion_registry_get_profile_artemisa_element_is_earth() -> void:
	var registry = _make_registry()
	var profile: Dictionary = registry.get_profile("artemisa")
...
func test_companion_registry_get_all_ids_contains_artemisa() -> void:
	var registry = _make_registry()
	var ids: Array[String] = registry.get_all_ids()
	assert_bool(ids.has("artemisa")).is_true()
...
func test_companion_registry_get_portrait_path_artemisa_neutral_has_correct_format() -> void:
...
func test_companion_registry_get_profile_returns_defensive_copy() -> void:
	# Arrange
	var registry = _make_registry()
	var profile_first: Dictionary = registry.get_profile("artemisa")
	...
	var profile_second: Dictionary = registry.get_profile("artemisa")
...
func test_companion_registry_get_profile_known_id_completes_within_one_ms() -> void:
	...
	var _profile: Dictionary = registry.get_profile("artemisa")
